...
class PongEnvironment(Environment):
    ...
    def reset(
        ...
        self.ball_vel = np.array([
            direction * self.ball_speed * np.cos(angle),
            self.ball_speed * np.sin(angle)
        ], dtype=np.float32)
    ...
    def step(
        ...
        if (self.ball_pos[0] - self.ball_radius <= 0.05 and
            abs(self.ball_pos[1] - self.paddle1_y) <= self.paddle_height / 2):
            self.ball_vel[0] = abs(self.ball_vel[0])
            # Add some randomness to the bounce
            self.ball_vel[1] += self.np_random.uniform(-0.01, 0.01)
        
        # Right paddle (player 2)
        if (self.ball_pos[0] + self.ball_radius >= 0.95 and
            abs(self.ball_pos[1] - self.paddle2_y) <= self.paddle_height / 2):
            self.ball_vel[0] = -abs(self.ball_vel[0])
            # Add some randomness to the bounce
            self.ball_vel[1] += self.np_random.uniform(-0.01, 0.01)
        ...
        terminated = (self.score1 >= self.winning_score or 
                     self.score2 >= self.winning_score)
